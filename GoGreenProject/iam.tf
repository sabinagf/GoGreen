...
#   for_each = toset(var.sysadmin_usernames)
...
# for_each = toset(var.dbadmin_usernames)

# user = aws_iam_user.dbadmin_users[each.value].name
...
#   for_each = toset(var.monitor_user_names)
